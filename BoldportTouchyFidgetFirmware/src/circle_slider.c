...
bool IsTouchQualified(void)
{
  ...
  if (CSLIB_isSensorDebounceActive(TS_CS0) ||
      CSLIB_isSensorDebounceActive(TS_CS1) ||
      CSLIB_isSensorDebounceActive(TS_CS2))
  {
    // For each sensor, calculate the delta between the current
    // capacitance value and baseline capacitance value, or delta
    for (sensor_index = TS_CS0; sensor_index <= TS_CS2; sensor_index++)
    {
...
uint16_t CalculatePosition()
{
    ...
    for (sensor_index = TS_CS0; sensor_index <= TS_CS2; sensor_index++)
    {
        if (CSLIB_node[sensor_index].processBuffer[0] < CSLIB_node[sensor_index].currentBaseline)
        {
            deltas[sensor_index - TS_CS0] = 0;
        }
        else
        {
            deltas[sensor_index - TS_CS0] = (CSLIB_node[sensor_index].processBuffer[0] - CSLIB_node[sensor_index].currentBaseline) >> 8;
        }
    }
